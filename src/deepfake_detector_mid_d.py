...
"""
    Deepfake Detector with RGB as input layer, D as mid layer
"""
...
class DeepfakeDetector(nn.Module):
    """
        Deepfake Detector Class (RGB Input, D Mid)
    ...
    def __init__(self):
        ...
        # MobileNet for RGB images - Split into two parts
        mobilenet = models.mobilenet_v2(weights=models.MobileNet_V2_Weights.IMAGENET1K_V1)
        self.mobilenet_stage1 = nn.Sequential(
            mobilenet.features[:10]  # Use the first 10 layers of MobileNet
        )

        # Replace the first convolutional layer in the 11th layer with a custom one that takes 96 channels as input
        new_conv = nn.Conv2d(96, mobilenet.features[10].conv[1].out_channels, kernel_size=1, stride=1)

        # Rebuild the 11th layer with the new convolutional layer
        self.mobilenet_stage2 = nn.Sequential(
            nn.Sequential(
                new_conv,
                mobilenet.features[10].conv[1:],  # Keep the rest of the layers in the 11th block
            ),
            *mobilenet.features[11:],  # Use the remaining layers of MobileNet
        ...
        self.depth_net = nn.Sequential(
            ...
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d((14, 14))  # Ensure output matches the size after mobilenet_stage1
        )
    ...
    def forward(self, images, depth_maps):
        # Process RGB images with the first part of MobileNet
        rgb_features = self.mobilenet_stage1(images)

        # Process depth maps with the small convolutional network
        depth_features = self.depth_net(depth_maps)

        # Concatenate the output of MobileNet's first part and the depth features
        fused_features = torch.cat((rgb_features, depth_features), dim=1)

        # Pass the combined features through the second part of MobileNet
        fused_features = self.mobilenet_stage2(fused_features)
...
def train_model(epochs=15):
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for images, depth_maps, labels in val_loader:
                ...
                val_acc += ((outputs > 0.5).float() == labels).float().mean().item()
    ...
    # Save the model
    torch.save(model.state_dict(), 'deepfake_detector_mid_d.pth')
...
def evaluate_model(test_loader=None):
    ...
    model = DeepfakeDetector()
    model.load_state_dict(torch.load('deepfake_detector_mid_d.pth'))
    model.eval()
    ...
    plt.title('Confusion Matrix - Middle Depth')
    textstr = '\n'.join((
    ...
    plt.savefig('plots/confusion_matrix_mid_d.png', dpi=300)
    plt.show()
    ...
    plt.title('ROC Curve - Middle Depth')
    plt.legend(loc="lower right")
    plt.savefig('plots/roc_curve_mid_d.png', dpi=300)
    plt.show()
